...
NSString * const kDefaultUsername = @"Michael";
...
- (void)didPressRightButton:(id)sender {
	...
	if (/* DISABLES CODE */ (YES)) {
        [MMXUser allUsersWithLimit:20 offset:0 success:^(int totalCount, NSArray *users) {
...
- (void)sendMessageTo:(NSArray *)users {
    [self.textView refreshFirstResponder];
    NSLog(@"%@", self.textView.text);
    MMXMessage *msg = [MMXMessage messageToRecipients:[NSSet setWithArray:users] messageContent:@{kTextContent:self.textView.text}];
